...
Description:
. The application is a simulation of a toy Turtle moving on a square tabletop, of dimensions 5 units x 5 units.
. There are no other obstructions on the table surface.
. The Turtle is free to roam around the surface of the table, but must be prevented from falling to destruction. Any movement
that would result in the Turtle falling from the table must be prevented, however further valid movement commands must still
be allowed.
...
. PLACE will put the toy Turtle on the table in position X,Y and facing NORTH, SOUTH, EAST or WEST.
...
. MOVE will move the toy Turtle one unit forward in the direction it is currently facing.
...
. A Turtle that is not on the table can choose the ignore the MOVE, LEFT, RIGHT and REPORT commands.
. Input can be from a file, or from standard input, as the developer chooses.
. Provide test data to exercise the application.


Constraints:
The toy Turtle must not fall off the table during movement. This also includes the initial placement of the toy Turtle.
